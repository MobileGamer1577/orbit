            ...
            children: [
                  ...
                  children: [
                    ...
                    IconButton(
                      ...
                      icon: _service.loading
                          ...
                          : Icon(Icons.refresh, color: Colors.white.withOpacity(0.80)),
                    ),
                  ],
                ),
              ),
              if (_service.data != null)
                Padding(
                  ...
                  child: Text(
                    'Aktualisiert: ${_formatTime(_service.data!.fetchedAt)} • ${_service.data!.entries.length} Einträge',
                    style: TextStyle(color: Colors.white.withOpacity(0.40), fontSize: 12),
                  ),
                ),

              // ── Debug-Panel (immer sichtbar bis Fix) ────
              if (_service.data != null && _service.data!.debugInfo.isNotEmpty)
                Container(
                  ...
                  decoration: BoxDecoration(
                    ...
                    border: Border.all(color: const Color(0xFF9C6FFF).withOpacity(0.5)),
                  ),
                  child: Text(
                    _service.data!.debugInfo,
                    style: const TextStyle(color: Colors.white60, fontSize: 10, fontFamily: 'monospace', height: 1.55),
                  ),
                ),
  ...
  Widget _buildBody() {
    if (_service.loading && _service.data == null) {
      return const Center(child: Column(mainAxisSize: MainAxisSize.min, children: [
        ...
        SizedBox(height: 16),
        Text('Shop wird geladen…', style: TextStyle(color: Colors.white54, fontSize: 15)),
      ]));
    }
    ...
    if (_service.data == null || _service.data!.entries.isEmpty) {
      return _ErrorView(error: 'Keine Einträge gefunden.', onRetry: _service.fetch);
    }
        ...
        return Column(
          ...
          children: [
            ...
            Text(sectionName.toUpperCase(),
                style: TextStyle(color: Colors.white.withOpacity(0.45), fontSize: 11, fontWeight: FontWeight.w700, letterSpacing: 1.5)),
            ...
            GridView.builder(
              ...
              itemBuilder: (context, i) => _ShopCard(entry: entries[i], imgMap: imgMap),
            ),
          ],
        );
      },
            ...
            Expanded(
              child: Stack(fit: StackFit.expand, children: [
                Container(decoration: BoxDecoration(gradient: LinearGradient(
                  ...
                  colors: [accentColor.withOpacity(0.22), const Color(0xFF07020F)],
                ))),
                if (imageUrl != null)
                  Image.network(imageUrl, fit: BoxFit.contain,
                    ...
                    loadingBuilder: (_, child, progress) => progress == null ? child
                        : Center(child: SizedBox(width: 22, height: 22,
                            child: CircularProgressIndicator(strokeWidth: 2, color: accentColor.withOpacity(0.60)))),
                  )
                else
                  const _NoImage(),
                if (entry.isOnSale)
                  Positioned(top: 7, right: 7, child: _Badge(label: 'SALE', color: Colors.red.shade600)),
                if (entry.isBundle)
                  Positioned(top: 7, left: 7, child: _Badge(label: 'BUNDLE', color: Colors.purple.shade700)),
              ]),
            ),
            Padding(
              padding: const EdgeInsets.fromLTRB(10, 8, 10, 10),
              child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
                Text(entry.displayName, maxLines: 2, overflow: TextOverflow.ellipsis,
                    style: const TextStyle(color: Colors.white, fontWeight: FontWeight.w800, fontSize: 13, height: 1.2)),
                if (entry.primaryTypeDisplay.isNotEmpty) ...[
                  const SizedBox(height: 2),
                  Text(entry.primaryTypeDisplay,
                      style: TextStyle(color: accentColor.withOpacity(0.85), fontSize: 11, fontWeight: FontWeight.w600)),
                ],
                ...
                Row(children: [
                  ...
                  if (entry.isOnSale) ...[
                    ...
                    Text('${entry.regularPrice}',
                        style: TextStyle(color: Colors.white.withOpacity(0.38), fontWeight: FontWeight.w600,
...
class _NoImage extends StatelessWidget {
  ...
  @override
  Widget build(BuildContext context) =>
      const Icon(Icons.image_not_supported_outlined, color: Colors.white12, size: 36);
}
...
class _VBucksIcon extends StatelessWidget {
  ...
  Widget build(BuildContext context) => Container(
    ...
    decoration: BoxDecoration(shape: BoxShape.circle,
      ...
      border: Border.all(color: const Color(0xFF00C8FF).withOpacity(0.60), width: 1),
    ),
    child: const Center(child: Text('V',
        style: TextStyle(color: Color(0xFF00C8FF), fontSize: 9, fontWeight: FontWeight.w900))),
  );
}
  ...
  Widget build(BuildContext context) => Center(child: Padding(
    ...
    child: Column(mainAxisSize: MainAxisSize.min, children: [
      ...
      Text('Shop konnte nicht geladen werden',
          style: TextStyle(color: Colors.white.withOpacity(0.75), fontSize: 17, fontWeight: FontWeight.w700),
          textAlign: TextAlign.center),
      const SizedBox(height: 8),
      Text(error, style: TextStyle(color: Colors.white.withOpacity(0.40), fontSize: 12), textAlign: TextAlign.center),
