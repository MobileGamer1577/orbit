...
class _FortniteAllCosmeticsScreenState
  ...
  void _onSearch() {
    ...
    _debounce = Timer(const Duration(milliseconds: 250), () {
      if (mounted) setState(() { _query = _searchCtrl.text.trim().toLowerCase(); _applyFilter(); });
    });
  }
  ...
  void _applyFilter() {
    if (_query.isEmpty) {
      ...
      _filtered = _items.where((item) =>
        ...
        item.typeDisplay.toLowerCase().contains(_query)).toList();
    }
  }

  Future<void> _loadCategory(_CosmeticCategory cat) async {
    if (_cache.containsKey(cat)) {
      if (mounted) setState(() { _items = _cache[cat]!; _applyFilter(); });
      return;
    }
    ...
    try {
      final res = await http.get(
        ...
        headers: {'Authorization': _apiKey, 'Accept': 'application/json', 'User-Agent': 'Orbit/1.0'},
      ...
      final items = list.whereType<Map<String, dynamic>>().map((m) {
        switch (cat) {
          case _CosmeticCategory.br:          return CosmeticItem.fromBrApi(m);
          case _CosmeticCategory.tracks:      return CosmeticItem.fromTrackApi(m);
          case _CosmeticCategory.instruments: return CosmeticItem.fromInstrumentApi(m);
          case _CosmeticCategory.cars:        return CosmeticItem.fromCarApi(m);
          case _CosmeticCategory.lego:        return CosmeticItem.fromLegoApi(m);
          case _CosmeticCategory.beans:       return CosmeticItem.fromBeanApi(m);
        }
      ...
      if (mounted) setState(() { _items = items; _applyFilter(); _loading = false; });
    } catch (e) {
        ...
        child: SafeArea(
          child: Column(
            ...
            children: [
              ...
              Padding(
                ...
                child: Row(
                  children: [
                    ...
                    Expanded(
                      child: Text(l10n.cosmeticsAll, style: const TextStyle(
              ...
              Expanded(
                child: _loading
                    ? const Center(child: CircularProgressIndicator(color: Color(0xFF9C6FFF)))
                    : _error != null
                        ? _ErrorWidget(error: _error!, onRetry: () => _loadCategory(_category))
                        : _filtered.isEmpty
                            ? Center(child: Text(
...
class _CosmeticCard extends StatelessWidget {
  ...
  const _CosmeticCard({required this.item, required this.collection, required this.onTap});

  @override
  Widget build(BuildContext context) {
    final accent = _rarityColor(item.rarityValue);
    final owned  = collection.isOwned(CollectionStore.categoryCosmetic, item.id);
    final wished = collection.isWished(CollectionStore.categoryCosmetic, item.id);
                ...
                child: Stack(
                  ...
                  children: [
                    ...
                    if (item.imageUrl != null)
                      Image.network(item.imageUrl!, fit: BoxFit.contain, cacheWidth: 300,
                    ...
                    if (owned || wished)
                      Positioned(
                        ...
                        child: Row(
                          ...
                          children: [
                            if (owned)  const Icon(Icons.check_circle, color: Color(0xFF00E676), size: 16),
                            if (wished) const Icon(Icons.favorite,     color: Color(0xFFFF4081), size: 16),
                          ],
                        ),
                      ),
                  ],
    ...
    child: Padding(
      ...
      child: Column(
        ...
        children: [
          ...
          Text('Konnte nicht laden', style: TextStyle(
              color: Colors.white.withOpacity(0.70), fontSize: 16, fontWeight: FontWeight.w700)),
          const SizedBox(height: 8),
          Text(error, style: TextStyle(color: Colors.white.withOpacity(0.35), fontSize: 11),
